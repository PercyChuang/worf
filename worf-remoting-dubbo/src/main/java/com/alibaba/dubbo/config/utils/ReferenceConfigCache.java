...
/**
 * a simple util class for cache {@link ReferenceConfig}. <p> {@link ReferenceConfig} is a heavy Object, it's necessary
 * to cache these object for the framework which create {@link ReferenceConfig} frequently. <p> You can implement and
 * use your own {@link ReferenceConfig} cache if you need use complicate strategy.
 *
 * @author ding.lid
 */
public class ReferenceConfigCache {
    ...
    /**
     * Get the cache use default name and {@link #DEFAULT_KEY_GENERATOR} to generate cache key. Create cache if not
     * existed yet.
     */
    ...
    /**
     * Get the cache use specified name and {@link KeyGenerator}. Create cache if not existed yet.
     */
    public static ReferenceConfigCache getCache(final String name) {
        return getCache(name, DEFAULT_KEY_GENERATOR);
    }

    /**
     * Get the cache use specified {@link KeyGenerator}. Create cache if not existed yet.
     */
    public static ReferenceConfigCache getCache(final String name, final KeyGenerator keyGenerator) {
     ...
     * <p> key example: <code>group1/com.alibaba.foo.FooService:1.0.0</code>.
     */
    public static final KeyGenerator DEFAULT_KEY_GENERATOR = new KeyGenerator() {
        @Override
        public String generateKey(final ReferenceConfig<?> referenceConfig) {
    ...
    private ReferenceConfigCache(final String name, final KeyGenerator generator) {
        this.name = name;
        this.generator = generator;
    }

    @SuppressWarnings("unchecked")
    public <T> T get(final ReferenceConfig<T> referenceConfig) {
    ...
    void destroyKey(final String key) {
     ...
     */
    public <T> void destroy(final ReferenceConfig<T> referenceConfig) {
        String key = generator.generateKey(referenceConfig);
        destroyKey(key);
    }
