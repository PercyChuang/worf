...
public class MonitorFilter implements Filter {
    ...
    public void setMonitorFactory(final MonitorFactory monitorFactory) {
    ...
    // 信息采集
    private void collect(final Invoker<?> invoker, final Invocation invocation, final Result result,
            RpcContext context, final long start, final boolean error) {
        try {
            // ---- 服务信息获取 ----
            ...
            monitor.collect(new URL(Constants.COUNT_PROTOCOL, NetUtils.getLocalHost(), localPort, service + "/"
                    + method, MonitorService.APPLICATION, application, MonitorService.INTERFACE, service,
                    MonitorService.METHOD, method, remoteKey, remoteValue, error ? MonitorService.FAILURE
                            : MonitorService.SUCCESS, "1", MonitorService.ELAPSED, String.valueOf(elapsed),
                    MonitorService.CONCURRENT, String.valueOf(concurrent), Constants.INPUT_KEY, input,
    ...
    // 获取并发计数器
    private AtomicInteger getConcurrent(final Invoker<?> invoker, final Invocation invocation) {
