...
public class FailoverClusterInvoker<T> extends AbstractClusterInvoker<T> {
    ...
    public FailoverClusterInvoker(final Directory<T> directory) {
        super(directory);
    }

    @Override
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public Result doInvoke(final Invocation invocation, final List<Invoker<T>> invokers, final LoadBalance loadbalance)
            throws RpcException {
        ...
        for (int i = 0; i < len; i++) {
            ...
            try {
                ...
                if (le != null && logger.isWarnEnabled()) {
                    logger.warn(
                            "Although retry the method " + invocation.getMethodName() + " in the service "
        ...
        throw new RpcException(le != null ? le.getCode() : 0, "Failed to invoke the method "
                + invocation.getMethodName() + " in the service " + getInterface().getName() + ". Tried " + len
                + " times of the providers " + providers + " (" + providers.size() + "/" + copyinvokers.size()
                + ") from the registry " + directory.getUrl().getAddress() + " on the consumer "
