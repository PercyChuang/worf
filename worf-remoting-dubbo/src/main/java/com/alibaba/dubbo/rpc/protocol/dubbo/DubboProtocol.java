...
public class DubboProtocol extends AbstractProtocol {
    ...
    private ExchangeHandler requestHandler = new ExchangeHandlerAdapter() {
        ...
        @Override
        public Object reply(final ExchangeChannel channel, final Object message) throws RemotingException {
            ...
            throw new RemotingException(channel, "Unsupported request: " + message == null ? null : message.getClass()
                    .getName()
                    + ": "
                    + message
                    + ", channel: consumer: "
        ...
        private void invoke(final Channel channel, final String methodKey) {
        ...
        private Invocation createInvocation(final Channel channel, final URL url, final String methodKey) {
    ...
    private boolean isClientSide(final Channel channel) {
        InetSocketAddress address = channel.getRemoteAddress();
        URL url = channel.getUrl();
        return url.getPort() == address.getPort()
                && NetUtils.filterLocalHost(channel.getUrl().getIp()).equals(
                        NetUtils.filterLocalHost(address.getAddress().getHostAddress()));
    }

    Invoker<?> getInvoker(final Channel channel, final Invocation inv) throws RemotingException {
    ...
    private void openServer(final URL url) {
        // find server.
    ...
    private ExchangeServer createServer(URL url) {
        ...
        url = url.addParameter(Constants.CODEC_KEY, Version.isCompatibleVersion() ? COMPATIBLE_CODEC_NAME
                : DubboCodec.NAME);
    ...
    private ExchangeClient[] getClients(final URL url) {
        // 是否共享连接
     ...
     * 获取共享连接
     */
    private ExchangeClient getSharedClient(final URL url) {
        String key = url.getAddress();
    ...
    private ExchangeClient initClient(URL url) {
        ...
        url = url.addParameter(Constants.CODEC_KEY, Version.isCompatibleVersion() && compatible ? COMPATIBLE_CODEC_NAME
                : DubboCodec.NAME);
        // 默认开启heartbeat
        url = url.addParameterIfAbsent(Constants.HEARTBEAT_KEY, String.valueOf(Constants.DEFAULT_HEARTBEAT));
