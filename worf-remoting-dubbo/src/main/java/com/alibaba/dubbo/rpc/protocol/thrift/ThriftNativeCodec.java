...
public class ThriftNativeCodec implements Codec2 {
    ...
    protected void encodeRequest(final Channel channel, final ChannelBuffer buffer, final Request request)
            throws IOException {
        Invocation invocation = (Invocation) request.getData();
        TProtocol protocol = newProtocol(channel.getUrl(), buffer);
        try {
            protocol.writeMessageBegin(new TMessage(invocation.getMethodName(), TMessageType.CALL, thriftSeq
                    .getAndIncrement()));
    ...
    protected void encodeResponse(final Channel channel, final ChannelBuffer buffer, final Response response)
            throws IOException {
    ...
    protected static TProtocol newProtocol(final URL url, final ChannelBuffer buffer) throws IOException {
