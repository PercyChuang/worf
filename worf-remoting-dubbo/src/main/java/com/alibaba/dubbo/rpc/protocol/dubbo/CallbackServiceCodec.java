...
class CallbackServiceCodec {
    ...
    private static byte isCallBack(final URL url, final String methodName, final int argIndex) {
    ...
    private static String exportOrunexportCallbackService(final Channel channel, final URL url, final Class clazz,
            final Object inst, final Boolean export) throws IOException {
        ...
        URL exporturl = new URL(DubboProtocol.NAME, channel.getLocalAddress().getAddress().getHostAddress(), channel
                .getLocalAddress().getPort(), clazz.getName() + "." + instid, tmpmap);
    ...
    private static Object referOrdestroyCallbackService(final Channel channel, final URL url, final Class<?> clazz,
            final Invocation inv, final int instid, final boolean isRefer) {
        Object proxy = null;
        ...
        if (isRefer) {
            if (proxy == null) {
                URL referurl = URL.valueOf("callback://" + url.getAddress() + "/" + clazz.getName() + "?"
                        + Constants.INTERFACE_KEY + "=" + clazz.getName());
                ...
                if (!isInstancesOverLimit(channel, referurl, clazz.getName(), instid, true)) {
                    ...
                    logger.info("method " + inv.getMethodName() + " include a callback service :" + invoker.getUrl()
    ...
    private static String getClientSideCallbackServiceCacheKey(final int instid) {
        return Constants.CALLBACK_SERVICE_KEY + "." + instid;
    }

    private static String getServerSideCallbackServiceCacheKey(final Channel channel, final String interfaceClass,
            final int instid) {
        return Constants.CALLBACK_SERVICE_PROXY_KEY + "." + System.identityHashCode(channel) + "." + interfaceClass
                + "." + instid;
    }

    private static String getServerSideCallbackInvokerCacheKey(final Channel channel, final String interfaceClass,
            final int instid) {
        return getServerSideCallbackServiceCacheKey(channel, interfaceClass, instid) + "." + "invoker";
    }

    private static String getClientSideCountKey(final String interfaceClass) {
        return Constants.CALLBACK_SERVICE_KEY + "." + interfaceClass + ".COUNT";
    }

    private static String getServerSideCountKey(final Channel channel, final String interfaceClass) {
        return Constants.CALLBACK_SERVICE_PROXY_KEY + "." + System.identityHashCode(channel) + "." + interfaceClass
                + ".COUNT";
    }

    private static boolean isInstancesOverLimit(final Channel channel, final URL url, final String interfaceClass,
            final int instid, final boolean isServer) {
    ...
    private static void increaseInstanceCount(final Channel channel, final String countkey) {
        try {
    ...
    private static void decreaseInstanceCount(final Channel channel, final String countkey) {
        try {
            Integer count = (Integer) channel.getAttribute(countkey);
    ...
    public static Object encodeInvocationArgument(final Channel channel, final RpcInvocation inv, final int paraIndex)
            throws IOException {
        // encode时可直接获取url
    ...
    public static Object decodeInvocationArgument(final Channel channel, final RpcInvocation inv, final Class<?>[] pts,
            final int paraIndex, final Object inObject) throws IOException {
