...
public class MockClusterInvoker<T> implements Invoker<T> {
    ...
    public MockClusterInvoker(final Directory<T> directory, final Invoker<T> invoker) {
    ...
    private Result doMockInvoke(final Invocation invocation, final RpcException e) {
        Result result = null;
        Invoker<T> minvoker;
    ...
    private String getMockExceptionMessage(final Throwable t, final Throwable mt) {
    ...
    /**
     * 返回MockInvoker 契约： directory根据invocation中是否有Constants.INVOCATION_NEED_MOCK，来判断获取的是一个normal invoker 还是一个 mock
     * invoker 如果directorylist 返回多个mock invoker，只使用第一个invoker.
     * 
     * @param invocation
     * @return
     */
    private List<Invoker<T>> selectMockInvoker(final Invocation invocation) {
