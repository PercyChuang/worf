...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
    ...
    public AbstractClusterInvoker(final Directory<T> directory) {
        this(directory, directory.getUrl());
    }

    public AbstractClusterInvoker(final Directory<T> directory, final URL url) {
        if (directory == null) {
    ...
    /**
     * 使用loadbalance选择invoker.</br> a)先lb选择，如果在selected列表中 或者 不可用且做检验时，进入下一步(重选),否则直接返回</br> b)重选验证规则：selected >
     * available .保证重选出的结果尽量不在select中，并且是可用的
     *
     * @param availablecheck 如果设置true，在选择的时候先选invoker.available == true
     * @param selected 已选过的invoker.注意：输入保证不重复
     *
     */
    protected Invoker<T> select(final LoadBalance loadbalance, final Invocation invocation,
            final List<Invoker<T>> invokers, final List<Invoker<T>> selected) throws RpcException {
    ...
    private Invoker<T> doselect(final LoadBalance loadbalance, final Invocation invocation,
            final List<Invoker<T>> invokers, final List<Invoker<T>> selected) throws RpcException {
        if (invokers == null || invokers.size() == 0) {
            return null;
        }
     ...
     * @throws RpcException
     */
    private Invoker<T> reselect(final LoadBalance loadbalance, final Invocation invocation,
            final List<Invoker<T>> invokers, final List<Invoker<T>> selected, final boolean availablecheck)
                    throws RpcException {
    ...
    protected void checkInvokers(final List<Invoker<T>> invokers, final Invocation invocation) {
        if (invokers == null || invokers.size() == 0) {
    ...
    protected List<Invoker<T>> list(final Invocation invocation) throws RpcException {
