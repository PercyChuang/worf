 ...
 *
 * <pre> |<- message header ->|<- message body ->|
 * +----------------+----------------------+------------------+---------------------------+------------------+ | magic
 * (2 bytes)|message size (4 bytes)|head size(2 bytes)| version (1 byte) | header | message body |
 * +----------------+----------------------+------------------+---------------------------+------------------+ |<-
 * message size ->| </pre>
 *
 * <p> <b>header fields in version 1</b> <ol> <li>string - service name</li> <li>long - dubbo request id</li> </ol> </p>
 *
...
public class ThriftCodec implements Codec2 {
    ...
    @Override
    public void encode(final Channel channel, final ChannelBuffer buffer, final Object message) throws IOException {
    ...
    private Object decode(final TProtocol protocol) throws IOException {
    ...
    private void encodeRequest(final Channel channel, final ChannelBuffer buffer, final Request request)
            throws IOException {
        ...
        String methodArgs = ExtensionLoader
                .getExtensionLoader(ClassNameGenerator.class)
                .getExtension(
                        channel.getUrl().getParameter(ThriftConstants.CLASS_NAME_GENERATOR_KEY,
                                ThriftClassNameGenerator.NAME)).generateArgsClassName(serviceName, inv.getMethodName());
    ...
    private void encodeResponse(final Channel channel, final ChannelBuffer buffer, final Response response)
            throws IOException {
        ...
        String resultClassName = ExtensionLoader
                ...
                .getExtension(
                        channel.getUrl().getParameter(ThriftConstants.CLASS_NAME_GENERATOR_KEY,
                                ThriftClassNameGenerator.NAME)).generateResultClassName(rd.serviceName, rd.methodName);
    ...
    static class RequestData {
        ...
        static RequestData create(final int id, final String sn, final String mn) {
