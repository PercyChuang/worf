...
public class HeaderExchanger implements Exchanger {
    ...
    @Override
    public ExchangeClient connect(final URL url, final ExchangeHandler handler) throws RemotingException {
        return new HeaderExchangeClient(
                Transporters.connect(url, new DecodeHandler(new HeaderExchangeHandler(handler))));
    }

    @Override
    public ExchangeServer bind(final URL url, final ExchangeHandler handler) throws RemotingException {
