...
public class ReplierDispatcher implements Replier<Object> {
    ...
    public ReplierDispatcher(final Replier<?> defaultReplier) {
        this(defaultReplier, null);
    }

    public ReplierDispatcher(final Replier<?> defaultReplier, final Map<Class<?>, Replier<?>> repliers) {
    ...
    public <T> ReplierDispatcher addReplier(final Class<T> type, final Replier<T> replier) {
        repliers.put(type, replier);
        return this;
    }

    public <T> ReplierDispatcher removeReplier(final Class<T> type) {
        repliers.remove(type);
        return this;
    }

    private Replier<?> getReplier(final Class<?> type) {
