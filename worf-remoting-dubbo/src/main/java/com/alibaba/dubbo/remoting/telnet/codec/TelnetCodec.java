...
public class TelnetCodec extends TransportCodec {
    ...
    private static final List<?> EXIT = Arrays.asList(new Object[] { new byte[] { 3 } /* Windows Ctrl+C */,
            new byte[] { -1, -12, -1, -3, 6 } /* Linux Ctrl+C */, new byte[] { -1, -19, -1, -3, 6 } /* Linux Pause */});

    @Override
    public void encode(final Channel channel, final ChannelBuffer buffer, Object message) throws IOException {
    ...
    protected Object decode(final Channel channel, final ChannelBuffer buffer, final int readable, byte[] message)
            throws IOException {
        if (isClientSide(channel)) {
        ...
        if (message[message.length - 1] == '\b') { // Windows backspace echo
            try {
                ...
                channel.send(new String(doublechar ? new byte[] { 32, 32, 8, 8 } : new byte[] { 32, 8 }, getCharset(
                        channel).name()));
    ...
    private static Charset getCharset(final Channel channel) {
        if (channel != null) {
    ...
    private static String toString(final byte[] message, final Charset charset) throws UnsupportedEncodingException {
    ...
    private static boolean isEquals(final byte[] message, final byte[] command) throws IOException {
        return message.length == command.length && endsWith(message, command);
    }

    private static boolean endsWith(final byte[] message, final byte[] command) throws IOException {
