...
public class RegistryProtocol implements Protocol {
    ...
    public void setRegistryFactory(final RegistryFactory registryFactory) {
        this.registryFactory = registryFactory;
    }

    private ProxyFactory proxyFactory;

    public void setProxyFactory(final ProxyFactory proxyFactory) {
     ...
     */
    @SuppressWarnings("unchecked")
    private <T> void doChangeLocalExport(final Invoker<T> originInvoker, final URL newInvokerUrl) {
        String key = getCacheKey(originInvoker);
    ...
    private URL getRegistedProviderUrl(final Invoker<?> originInvoker) {
        ...
        // 注册中心看到的地址
        final URL registedProviderUrl = providerUrl.removeParameters(getFilteredKeys(providerUrl)).removeParameter(
                Constants.MONITOR_KEY);
        return registedProviderUrl;
    }

    private URL getSubscribedOverrideUrl(final URL registedProviderUrl) {
    ...
    public <T> Invoker<T> refer(final Class<T> type, URL url) throws RpcException {
        url = url.setProtocol(url.getParameter(Constants.REGISTRY_KEY, Constants.DEFAULT_REGISTRY)).removeParameter(
    ...
    private <T> Invoker<T> doRefer(final Cluster cluster, final Registry registry, final Class<T> type, final URL url) {
        RegistryDirectory<T> directory = new RegistryDirectory<T>(type, url);
        directory.setRegistry(registry);
        directory.setProtocol(protocol);
        URL subscribeUrl = new URL(Constants.CONSUMER_PROTOCOL, NetUtils.getLocalHost(), 0, type.getName(), directory
    ...
    private static String[] getFilteredKeys(final URL url) {
    ...
    /*
     * 重新export 1.protocol中的exporter destory问题1.要求registryprotocol返回的exporter可以正常destroy2.notify后不需要重新向注册中心注册3.export
    ...
    private class OverrideListener implements NotifyListener {
        ...
        public OverrideListener(final URL subscribeUrl) {
            this.subscribeUrl = subscribeUrl;
        }

        /*
         * provider 端可识别的override url只有这两种. override://0.0.0.0/serviceName?timeout=10 override://0.0.0.0/?timeout=10
         */
        @Override
        public void notify(List<URL> urls) {
            ...
            for (URL url : urls) {
                ...
                if (!UrlUtils.isMatch(subscribeUrl, overrideUrl)) {
                    ...
                    result.remove(url);
                    logger.warn("Subsribe category=configurator, but notifed non-configurator urls. may be registry bug. unexcepted url: "
                            + url);
                }
            }
            if (result != null) {
        ...
        private URL getNewInvokerUrl(URL url, final List<URL> urls) {
    ...
    public static class InvokerDelegete<T> extends InvokerWrapper<T> {
         ...
         */
        public InvokerDelegete(final Invoker<T> invoker, final URL url) {
            super(invoker, url);
    ...
    private class ExporterChangeableWrapper<T> implements Exporter<T> {
        ...
        public ExporterChangeableWrapper(final Exporter<T> exporter, final Invoker<T> originInvoker) {
        ...
        public void setExporter(final Exporter<T> exporter) {
            this.exporter = exporter;
        }
