...
public abstract class FailbackRegistry extends AbstractRegistry {
    ...
    private final ScheduledExecutorService retryExecutor = Executors.newScheduledThreadPool(1, new NamedThreadFactory(
            "DubboRegistryFailedRetryTimer", true));
    ...
    public FailbackRegistry(final URL url) {
        super(url);
        int retryPeriod = url
                .getParameter(Constants.REGISTRY_RETRY_PERIOD_KEY, Constants.DEFAULT_REGISTRY_RETRY_PERIOD);
    ...
    private void addFailedSubscribed(final URL url, final NotifyListener listener) {
    ...
    private void removeFailedSubscribed(final URL url, final NotifyListener listener) {
        Set<NotifyListener> listeners = failedSubscribed.get(url);
        if (listeners != null) {
    ...
    protected void doNotify(final URL url, final NotifyListener listener, final List<URL> urls) {
