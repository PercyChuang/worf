...
public class GenericObjectOutput extends GenericDataOutput implements ObjectOutput {
    ...
    public GenericObjectOutput(final OutputStream out) {
        this(out, Builder.DEFAULT_CLASS_DESCRIPTOR_MAPPER);
    }

    public GenericObjectOutput(final OutputStream out, final ClassDescriptorMapper mapper) {
        super(out);
        mMapper = mapper;
        isAllowNonSerializable = false;
    }

    public GenericObjectOutput(final OutputStream out, final int buffSize) {
        this(out, buffSize, Builder.DEFAULT_CLASS_DESCRIPTOR_MAPPER, false);
    }

    public GenericObjectOutput(final OutputStream out, final int buffSize, final ClassDescriptorMapper mapper) {
        this(out, buffSize, mapper, false);
    }

    public GenericObjectOutput(final OutputStream out, final int buffSize, final ClassDescriptorMapper mapper,
            final boolean isAllowNonSerializable) {
        super(out, buffSize);
        mMapper = mapper;
    ...
    public int getRef(final Object obj) {
