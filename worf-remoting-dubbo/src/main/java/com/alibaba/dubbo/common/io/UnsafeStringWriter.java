...
public class UnsafeStringWriter extends Writer {
    ...
    public UnsafeStringWriter(final int size) {
        if (size < 0) {
    ...
    @Override
    public void write(final char[] cs) throws IOException {
        mBuffer.append(cs, 0, cs.length);
    }

    @Override
    public void write(final char[] cs, final int off, final int len) throws IOException {
