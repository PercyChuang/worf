...
/**
 * Default data output impl. Not thread-safe.
 * 
 * @author qian.lei
 */
...
public class GenericDataOutput implements DataOutput, GenericDataFlags {
    ...
    public GenericDataOutput(final OutputStream out) {
        this(out, 1024);
    }

    public GenericDataOutput(final OutputStream out, final int buffSize) {
        mOutput = out;
        mLimit = buffSize;
    ...
    protected void write0(final byte b) throws IOException {
        if (mPosition == mLimit) {
            flushBuffer();
        }
    ...
    protected void write0(final byte[] b, int off, int len) throws IOException {
