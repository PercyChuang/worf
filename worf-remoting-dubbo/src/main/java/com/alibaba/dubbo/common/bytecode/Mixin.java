...
public abstract class Mixin {
     ...
     */
    public static Mixin mixin(final Class<?>[] ics, final Class<?> dc) {
     ...
     * @return Mixin instance.
     */
    public static Mixin mixin(final Class<?>[] ics, final Class<?> dc, final ClassLoader cl) {
     ...
     * @return Mixin instance.
     */
    public static Mixin mixin(final Class<?>[] ics, final Class<?>[] dcs) {
     ...
     * @return Mixin instance.
     */
    public static Mixin mixin(final Class<?>[] ics, final Class<?>[] dcs, final ClassLoader cl) {
        ...
        try {
            ...
            for (int i = 0; i < dcs.length; i++) {
                ...
                code.append("d").append(i).append(" = (").append(dcs[i].getName()).append(")$1[").append(i)
                        .append("];\n");
            ...
            for (int i = 0; i < ics.length; i++) {
                ...
                for (Method method : ics[i].getMethods()) {
                    ...
                    if (Void.TYPE.equals(rt)) {
                        ccp.addMethod(mn, method.getModifiers(), rt, method.getParameterTypes(),
                                method.getExceptionTypes(), "d" + ix + "." + mn + "($$);");
                    } else {
                        ccp.addMethod(mn, method.getModifiers(), rt, method.getParameterTypes(),
                                method.getExceptionTypes(), "return ($r)d" + ix + "." + mn + "($$);");
                    }
                }
            }
    ...
    private static int findMethod(final Class<?>[] dcs, final String desc) {
        Class<?> cl;
        Method[] methods;
        for (int i = 0; i < dcs.length; i++) {
            cl = dcs[i];
    ...
    private static void assertInterfaceArray(final Class<?>[] ics) {
        for (int i = 0; i < ics.length; i++) {
